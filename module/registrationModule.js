    ...
    addRegistration: async function (
        ...
        try {
            ...
            if (eventData[0].eventFee == 0) {
                ...
                [currentRegistrationPerDepartment] = await db.query(
                    "SELECT CONVERT(SUM(numRegistrations), UNSIGNED) AS numRegistrations, (maxRegistrations) as maxRegistrations FROM deptEventMapping WHERE eventID = ? GROUP BY eventID",
                    [eventID],
                );
            }
            await db.query("UNLOCK TABLES");
            ...
            // Group Event.
                ...
                if (userTeamDataCheck.length !== teamMembers.length) {
                    return setResponseBadRequest(
                        "Failed to Register. Invalid Email, one of the Team Member Has not Registered for Amritotsavam 2025.",
                    );
                }
